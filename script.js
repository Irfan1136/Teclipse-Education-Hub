...
// ================================
// --- ✅ FIX: Star Rating — Gold Color on Hover & Click ---
...
function highlightStars(value) {
    // ✅ parseInt ensures numeric comparison, not string comparison
    ...
    stars.forEach(star => {
        ...
        if (starVal <= numVal) {
            ...
            star.style.color = '#f5c518'; // Gold
        } else {
            star.classList.remove('bxs-star');
            star.classList.add('bx-star');
            star.style.color = '#888'; // Grey
        }
    });
}
...
// ================================
// --- ✅ FIX: Render Achievements — Correctly reads base64 image from Firestore ---
function renderMainAchievements(achievements) {
    ...
    if (!achievements || achievements.length === 0) {
        gallery.innerHTML = `<p style="color:var(--text-muted); text-align:center; grid-column:1/-1; padding:2rem 0;">No achievements uploaded yet.</p>`;
        return;
    }
    ...
    achievements.forEach(a => {
        ...
        // ✅ Try all possible field names admin.js might have saved the image under
        const imgSrc = a.image || a.imageUrl || a.imageData || a.base64 || '';

        // ✅ Caption tries all possible field names too
        const caption = a.caption || a.title || a.description || a.desc || '';

        div.innerHTML = `
            <div style="position:relative; width:100%; height:200px; background:rgba(30,58,138,0.2); overflow:hidden;">
                <img
                    src="${imgSrc}"
                    alt="${caption}"
                ...
                ">
                    <i class='bx bx-image' style="font-size:2.5rem; color:#aaa;"></i>
                    <span style="color:#aaa; font-size:0.8rem;">Image unavailable</span>
                </div>
            </div>
            <h4 style="
...
function renderTestimonials(feedbacks) {
    ...
    // ✅ Always re-init AFTER slides injected
...
if (feedbackForm) {
    feedbackForm.addEventListener('submit', async e => {
        ...
        try {
            ...
            // ✅ Reset star colors after submit
